...
export function CoachVideoReviewSwitcher({ videos, submissionId, initialRecordingUrl }: { videos: ReviewVideo[]; submissionId: string; initialRecordingUrl?: string }) {
  ...
  return <div>
    ...
    <PilotVideoReviewTool key={videos[activeIndex].url} videoUrl={videos[activeIndex].url} reviewId={submissionId} initialRecordingUrl={recordingUrl} onRecordingSaved={setRecordingUrl} onRecordingDeleted={() => setRecordingUrl(undefined)} />
  </div>;
}
